...
/**
 * Synchronises Moodle course content with a Google Gemini File Search store.
 *
 ...
 */
class google_course_file_search_store_service {
    /** @var google_file_search_store_api File Search Store API. */
    private google_file_search_store_api $storeapi;
    ...
    public function __construct(?google_client $client = null) {
        ...
        $this->storeapi = new google_file_search_store_api($client);
    }

    /**
     * Ensures that a File Search store exists for a course.
     *
     * @param int $courseid Course id.
     * @return \stdClass Local vector store record.
     */
    public function ensure_file_search_store(int $courseid): \stdClass {
        global $CFG, $DB;
        ...
        if ($record) {
            if ($this->remote_store_exists((string)$record->externalid)) {
                return $record;
            }

            $this->reset_missing_store($record);
        ...
        $created = $this->storeapi->create($name, (string)get_config('local_studybuddy', 'googlefilestoreembeddingmodel'));
        $now = time();
        ...
        $record = (object)[
            ...
            'name' => $name,
            'status' => $this->status_from_store($created),
    ...
    public function delete_course_resources(int $courseid): void {
        ...
        try {
            if (!empty($store->externalid)) {
                $this->storeapi->delete((string)$store->externalid, true);
            }
        } catch (\Throwable $e) {
            unset($e);
        }
    ...
    /**
     * Synchronises all indexed course documents to Google.
     *
     * @param int $courseid Course id.
    ...
    public function sync_course(int $courseid): array {
        ...
        // Keep only source metadata locally. Gemini File Search owns parsing,
        // chunking, embeddings and retrieval.
        $documents = (new course_source_catalogue())->synchronise($courseid);
        $store = $this->ensure_file_search_store($courseid);
        $summary['vector_store_id'] = $store->externalid;
        ...
        try {
            ...
            foreach ($documents as $document) {
                ...
                if (!$existing) {
                    if ($this->upload_document($store, $document)) {
                        $summary['uploaded']++;
                    }
                    continue;
                }
            ...
            $summary['deleted'] += $this->delete_stale_files($store, $seen);
            $this->refresh_file_statuses($store, $summary);
            $remote = $this->storeapi->retrieve($store->externalid);
            $this->update_store_status($store, $remote, null);
            $summary['vector_store_status'] = $this->status_from_store($remote);
        } catch (\Throwable $e) {
            $this->update_store_status($store, null, $e->getMessage());
            throw $e;
        }
    ...
    /**
     * Removes Google File Search documents for a deleted course module.
     *
    ...
    /**
     * Checks whether a remote store still exists.
     *
     * @param string $storeid Store resource name.
     * @return bool
     */
    private function remote_store_exists(string $storeid): bool {
        try {
            $this->storeapi->retrieve($storeid);
            return true;
    ...
    /**
     * Clears local mappings for a store that has disappeared remotely.
     *
     * @param \stdClass $store Local store record.
     * @return void
     */
    private function reset_missing_store(\stdClass $store): void {
        global $DB;
    ...
    /**
     * Detects Google "resource not found" errors without hiding other failures.
     *
    ...
    /**
     * Uploads a single local document and records the indexing operation.
     *
     * @param \stdClass $store Local store record.
     * @param \stdClass $document Document record.
     * @return bool True when uploaded.
     */
    private function upload_document(\stdClass $store, \stdClass $document): bool {
        global $DB;

        $tmpdir = make_temp_directory('local_studybuddy_google');
        ...
        try {
            $operation = $this->storeapi->upload_document(
                $store->externalid,
                $upload['path'],
                $upload['filename'],
                $upload['mimetype'],
                [
                    [
                        'key' => 'moodle_documentid',
                        'numericValue' => (int)$document->id,
                    ],
                    [
                        'key' => 'moodle_cmid',
                        'numericValue' => (int)$document->cmid,
                    ],
                ]
            );
            $attributes = $this->document_attributes($document);
            $status = !empty($operation['done']) ? 'completed' : 'in_progress';
            $documentname = $operation['response']['name'] ?? null;
            $now = time();
    ...
    /**
     * Refreshes status for in-progress indexing operations.
     *
     * @param \stdClass $store Local store record.
     * @param array $summary Summary passed by reference.
     * @return void
     */
    private function refresh_file_statuses(\stdClass $store, array &$summary): void {
        global $DB;

        $records = $DB->get_records('local_studybuddy_store_files', ['vectorstoreid' => $store->id]);
        foreach ($records as $record) {
            ...
            try {
                $operation = $this->storeapi->get_operation($record->externalfileid);
     ...
     */
    private function delete_stale_files(\stdClass $store, array $seen): int {
        global $DB;
    ...
    /**
     * Best-effort deletion of a remote File Search document.
     *
    ...
    private function delete_remote_file_mapping(\stdClass $record): void {
        ...
        try {
            $this->storeapi->delete_document((string)$record->vectorfileid);
        } catch (\Throwable $e) {
            unset($e);
        }
    }
     ...
     * @return void
     */
    private function update_store_status(\stdClass $store, ?array $remote, ?string $error): void {
        global $DB;

        $update = (object)[
            'id' => $store->id,
            'status' => $remote ? $this->status_from_store($remote) : ($error ? 'failed' : $store->status),
            'metadata' => $store->metadata,
            'lastsynced' => $error ? $store->lastsynced : time(),
    ...
    /**
     * Converts a Google store response to local status.
     *
     * @param array $store Remote store.
     * @return string
     */
    private function status_from_store(array $store): string {
        if ((int)($store['failedDocumentsCount'] ?? 0) > 0) {
            return 'failed';
        }

        if ((int)($store['pendingDocumentsCount'] ?? 0) > 0) {
            return 'in_progress';
        }
